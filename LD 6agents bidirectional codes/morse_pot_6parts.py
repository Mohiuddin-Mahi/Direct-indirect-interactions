...
for epi_ind in range(1,len(epi_array)+1):
    ...
    g_array=np.linspace(0.4,2,9)
    
    for g_ind in range(1,len(g_array)+1):       #len(g_array)
    
        kT_array=np.linspace(0.6,3.8,9)
        ...
        for kT_ind in range(1,len(kT_array)+1):  
            ...
            def simu(arg):
                ...
                lbox = 10
                nparticles=6                          #int(nlattice[0])*int(nlattice[1])
                ...
                hoomd.init.create_lattice(unitcell=hoomd.lattice.unitcell(N=nparticles,a1=[lbox,0,0],a2=[0,lbox,0],a3=[0,0,1]\
                     ,dimensions=2,type_name = ['A', 'B', 'C','D','E','F'],position=[[lbox*(random.random()-0.5),lbox*(random.random()-0.5),0]\
                ...
                morse.pair_coeff.set('A', 'C', D0=epi_in, alpha=alpha_in, r0=re, r_cut=r_cut_in)                  # particle A and C have no interaction
                morse.pair_coeff.set('A', 'D', D0=epi_in, alpha=alpha_in, r0=re, r_cut=r_cut_in)
                morse.pair_coeff.set('A', 'E', D0=0.00, alpha=alpha_in, r0=re, r_cut=r_cut_in)
                morse.pair_coeff.set('A', 'F', D0=0.00, alpha=alpha_in, r0=re, r_cut=r_cut_in)
                morse.pair_coeff.set('B', 'B', D0=epi_in, alpha=alpha_in, r0=re, r_cut=r_cut_in)
                morse.pair_coeff.set('B', 'C', D0=0.00, alpha=alpha_in, r0=re, r_cut=r_cut_in)               # particle B and C have a harmonic interaction
                morse.pair_coeff.set('B', 'D', D0=epi_in, alpha=alpha_in, r0=re, r_cut=r_cut_in)
                morse.pair_coeff.set('B', 'E', D0=epi_in, alpha=alpha_in, r0=re, r_cut=r_cut_in)
                morse.pair_coeff.set('B', 'F', D0=0.00, alpha=alpha_in, r0=re, r_cut=r_cut_in)
                morse.pair_coeff.set('C', 'C', D0=epi_in, alpha=alpha_in, r0=re, r_cut=r_cut_in)
                morse.pair_coeff.set('C', 'D', D0=epi_in, alpha=alpha_in, r0=re, r_cut=r_cut_in)
                morse.pair_coeff.set('C', 'E', D0=0.00, alpha=alpha_in, r0=re, r_cut=r_cut_in)
                morse.pair_coeff.set('C', 'F', D0=epi_in, alpha=alpha_in, r0=re, r_cut=r_cut_in)
                morse.pair_coeff.set('D', 'D', D0=epi_in, alpha=alpha_in, r0=re, r_cut=r_cut_in)
                morse.pair_coeff.set('D', 'E', D0=epi_in, alpha=alpha_in, r0=re, r_cut=r_cut_in)
                morse.pair_coeff.set('D', 'F', D0=epi_in, alpha=alpha_in, r0=re, r_cut=r_cut_in)
                morse.pair_coeff.set('E', 'E', D0=epi_in, alpha=alpha_in, r0=re, r_cut=r_cut_in)
                morse.pair_coeff.set('E', 'F', D0=epi_in, alpha=alpha_in, r0=re, r_cut=r_cut_in)
                morse.pair_coeff.set('F', 'F', D0=epi_in, alpha=alpha_in, r0=re, r_cut=r_cut_in)
                morse.enable()
                ...
                obj1.set_gamma(a='D',gamma=gamma_in)
                obj1.set_gamma(a='E',gamma=gamma_in)
                obj1.set_gamma(a='F',gamma=gamma_in)
                ...
                sio.savemat('result_6_parts/l_10_epi_'+str(epi_in)+'_g_'+str(g_ind)+'_dt_'+str(d_t)+'_kT_'+str(kT_ind)+'_trail_'+str(seed_in)+'.mat',\
                            {'vx':vx,'vy':vy})
