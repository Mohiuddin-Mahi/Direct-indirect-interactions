...
function stat_test_true_surr_data_standard_te(epi,dt,num_trail,num_kT,nparticles,num_gamma,num_symbols,num_tau)
...
for ind1=1:nparticles
    for ind2=1:nparticles
        if ind1~=ind2
                    ...
                    for tau_ind=1:num_tau
                        ...
                        for trail1=1:num_trail
                            for trail2=1:num_trail
                                if trail1==trail2
                                    true_te(trail1,tau_ind)=tran_ent_func_mohi(data.data_sym{trail2}(ind2,1:min_length),...
                                        data.data_sym{trail1}(ind1,1:min_length),num_symbols,tau_ind);
                                else
                                    surr_te(ind,tau_ind)=tran_ent_func_mohi(data.data_sym{trail2}(ind2,1:min_length),...
                                        data.data_sym{trail1}(ind1,1:min_length),num_symbols,tau_ind);
                                    ind=ind+1;
                                end
                            end
            ...
            save(['data_te/te_data_true_surr_part_',num2str(ind1),'_',num2str(ind2),...
