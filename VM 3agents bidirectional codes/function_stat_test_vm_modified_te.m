...
function function_stat_test_vm_modified_te(d,nbird,nsymbols,num_trail,num_noise,num_coupling,tau_max)
...
for bird1=1:nbird
    for bird2=1:nbird
        if bird1~=bird2
            ...
            for coup_ind=1:num_coupling
                for noise_ind=1:num_noise
                    ...
                    for tau_ind=1:tau_max
                        ...
                        temp_true_te=zeros(num_trail,1);
                        temp_surr_te=[];
                        
                        parfor trail1=1:num_trail
                            for trail2=1:num_trail
                                if trail1==trail2
                                    temp_true_te(trail1)=tran_ent_func_mohi_new(symbols{trail2}(bird2,d:end),...
                                        symbols{trail1}(bird1,d:end),nsymbols,tau);
                                else
                                    temp=tran_ent_func_mohi_new(symbols{trail2}(bird2,d:end),...
                                        symbols{trail1}(bird1,d:end),nsymbols,tau);
                                    temp_surr_te=[temp_surr_te;temp];
                                end
                            end
                        end
                        true_te(:,tau_ind)=temp_true_te;
                        surr_te(:,tau_ind)=temp_surr_te;
                    end
                    toc;
            ...
            save(['data_te/mt_data_stat_test_bird_',num2str(bird1),'_',num2str(bird2),...
